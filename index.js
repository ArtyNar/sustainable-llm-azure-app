  ...
  try {
    ...
    else
    {
      ...
      `<li class="list-group-item">
        ...
        <small>Status: <span class="badge badge-pill bg-warning text-dark">${item.status}</span> <br> Carbon (schedule time) : ${item.carbonIntensity_S} <br> Carbon (execution time) : ${item.carbonIntensity_C} <hr> ${item.prompt}</small>
      </li>`
    ).join('');
